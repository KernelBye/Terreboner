...
static Vec2 rotate_robot_to_field_offset(float heading_deg, float x_right, float y_fwd) {
  // For EZ-Template heading (same as JAR):
...
struct Meas {
  ...
  // Apply to odometry (EZ-Template uses odom_xyt_set instead of set_position)
  odom.odom_xyt_set(best_pose.x, best_pose.y, heading_deg);
